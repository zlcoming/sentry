    ...
    while i < len(args):
        if isinstance(args[i], (list, tuple)):
            ...
            if i < len(args) - 1 and isinstance(args[i + 1], (list, tuple)):
                ...
                i += 1
            elif args[i] not in derived_columns or args[i].startswith("metrics."):
            ...
            if aggregation[1].startswith("metrics."):
                ...
                for col in selected_columns:
                    if col[2].strip("`") == aggregation[1]:
                        ...
                        continue
                if not found:
                    fmt_str = "(toFloat32OrNull(arrayElement(tags.value, indexOf(tags.key, '{}'))))"
                    aggregation[0] += fmt_str.format(aggregation[1])
                    aggregation[1] = None
            else:
