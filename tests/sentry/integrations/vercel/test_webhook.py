...
from .testutils import EXAMPLE_DEPLOYMENT_WEBHOOK_RESPONSE, DEPLOYMENT_WEBHOOK_NO_TARGET
...
class VercelReleasesTest(APITestCase):
    ...
    def test_no_target(self):
        local_signature = hmac.new(
            ...
            msg=six.binary_type(DEPLOYMENT_WEBHOOK_NO_TARGET),
            digestmod=hashlib.sha1,
        ).hexdigest()

        response = self.client.post(
            path=webhook_url,
            data=DEPLOYMENT_WEBHOOK_NO_TARGET,
            content_type="application/json",
